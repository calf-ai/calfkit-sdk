...
- **[How to give agents discoverable tool nodes](docs/tool-discovery.md)** — reference deployed function tool nodes by name with `Tools`; agents discover their schemas at runtime, so an agent's deployment never imports the tool's code.
