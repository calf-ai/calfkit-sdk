"""Deploy the launch review — release_manager + engineering, security, legal — on one worker.
...
async def main() -> None:
    ...
    worker = Worker(client, nodes=NODES)  # the release manager + three experts + their tools, one worker
    await worker.run()
