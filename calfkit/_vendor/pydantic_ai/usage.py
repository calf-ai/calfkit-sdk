...
class UsageBase:
    input_tokens: Annotated[
        int,
        # `request_tokens` is deprecated, but we still want to support deserializing
        # model responses stored in a DB before the name was changed
    ...
    output_tokens: Annotated[
        int,
        # `response_tokens` is deprecated, but we still want to support deserializing
        # model responses stored in a DB before the name was changed
    ...
    details: Annotated[
        dict[str, int],
        # `details` can not be `None` any longer, but we still want to support deserializing
        # model responses stored in a DB before this was changed
...
class RequestUsage(UsageBase):
    ...
    This is an implementation of `genai_prices.types.AbstractUsage`
    so it can be used to calculate the price of the
    ...
    def __add__(self, other: RequestUsage) -> RequestUsage:
        ...
        **WARNING:** this CANNOT be used to sum multiple requests
    ...
    def extract(
        ...
        *,
        ...
        Args:
            ...
            provider_fallback: The fallback provider ID to use
            if the actual provider is not found in genai-prices.
                For example, an OpenAI model should set this to "openai"
                in case it has an obscure provider ID.
        ...
        for provider_id, provider_api_url in [
            ...
            try:
                ...
                _model_ref, extracted_usage = provider_obj.extract_usage(
                    data, api_flavor=api_flavor
                )
                return cls(
...
class RunUsage(UsageBase):
    ...
    Responsibility for calculating request usage is on the model;
    Pydantic AI simply sums the usage information across requests.
    """
...
class UsageLimits:
    ...
    The request count is tracked by pydantic_ai,
    and the request limit is checked before each request to the model.
    Token counts are provided in responses from the model,
    and the token limits are checked after each response.
    ...
    This may incur additional overhead (from calling the model's `count_tokens`
    API before making the actual request) and is disabled by default.
    ...
    @overload
    @deprecated(
        "Use `input_tokens_limit` instead of `request_tokens_limit` and"
        " `output_tokens_limit` and `total_tokens_limit`"
    )
    def __init__(
        self,
        *,
    ...
    def has_token_limits(self) -> bool:
        ...
        This is useful because if we have token limits,
        we need to check them after receiving each streamed message.
    ...
    def check_before_request(self, usage: RunUsage) -> None:
        """Raises a `UsageLimitExceeded` exception if the next
        request would exceed any of the limits."""
        ...
        if self.input_tokens_limit is not None and input_tokens > self.input_tokens_limit:
            raise UsageLimitExceeded(
                "The next request would exceed the input_tokens_limit of "
                f"{self.input_tokens_limit} ({input_tokens=})"
            )

        total_tokens = usage.total_tokens
        if self.total_tokens_limit is not None and total_tokens > self.total_tokens_limit:
            raise UsageLimitExceeded(  # pragma: lax no cover
                "The next request would exceed the total_tokens_limit of"
                f" {self.total_tokens_limit} ({total_tokens=})"
            )
    ...
    def check_before_tool_call(self, projected_usage: RunUsage) -> None:
        """Raises a `UsageLimitExceeded` exception if the next tool call(s)
        would exceed the tool call limit."""
        tool_calls_limit = self.tool_calls_limit
        tool_calls = projected_usage.tool_calls
        if tool_calls_limit is not None and tool_calls > tool_calls_limit:
            raise UsageLimitExceeded(
                "The next tool call(s) would exceed the tool_calls_limit of "
                f"{tool_calls_limit} ({tool_calls=})."
            )
