...
class TailCall(Generic[StateT], _Call[StateT]):
    ...
    publish chokepoint — set only on a genuine HANDOFF (handoff-tool-transport-spec §5/C2), where the tailcallee is a DIFFERENT agent
    and must use its own tools/model, not the caller's per-invocation ``tool_overrides``/``model_settings``.
    Default ``False`` PRESERVES ``frame.overrides`` (correct for the all-invalid / all-invalid self-retry,
