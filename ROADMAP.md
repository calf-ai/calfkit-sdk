...
- **Native toolbox node** — the non-MCP twin of `MCPToolboxNode`: one deployable node hosting many `@tool`-decorated methods (registry-collected), advertising N bindings on one topic. Sketched during the ToolBinding design sessions; would reuse `ToolProvider` as-is.
