...
| **Fault group** | The fault minted when a fan-out batch closes with unhandled sibling faults (`error_type="calf.fault_group"`, children in `causes`). Itself a fault; singletons flatten (§4.4). Not every fault with `causes` is a group — conversion, recovery-failure, and an uncaught exception's `__cause__` chain also chain causes. |
