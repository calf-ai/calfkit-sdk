...
- **Custom output-tool name** — the structured-answer surface matches the auto-generated `final_result*` namespace, i.e. `final_result` (single output) and `final_result_<Type>` (multi-output union); both are surfaced (§5.5). Only an agent that *overrides* the name via `ToolOutput(Model, name=...)` falls outside the namespace and will not have its structured answer surfaced cross-agent. The `final_result*` prefix is correspondingly **reserved**: a *function* tool a user names `final_result_*` would be matched and mis-surfaced as an answer — keep function-tool names out of that namespace. Both edges are removable later by resolving the tool name from the agent's output schema (the same follow-up); out of scope for v1.
