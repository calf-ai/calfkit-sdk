...
The closure is a self-published re-entry — the durable analog of fault-rail scenario 47, what §7.7 binds to. **Recognition precedes stray detection** (the exemption): `in_reply_to == fanout_id` resolves the node's **own** fan-out frame (no sibling slot equals it), so it's the closure, never a stray. *(⚠️ CORRECTION (PR-6 / 2026-06-17): this recognition is implemented in a dedicated `_classify_fanout` method, **NOT** as a behavior change to the sealed `_stray_check` — the closure re-entry is a shape-valid `return` + `ReturnMessage`, so it **passes** `_stray_check`'s kind↔slot-shape test and is then classified as a re-entry by `_classify_fanout`; it is stray-exempt by **routing**. See §10. The sealed `_stray_check` is unchanged.)*

**Stage 0 — rebuild the closure context.** After the standard `prepare_context` (which copies the empty wire `state`/`deps` and stamps the frame's `overrides`), **overwrite `ctx.state` from `basestate[X].snapshot`** (`barrier`-on-miss read) *before any seam runs*: `ctx.state` ← `snapshot.state`, stack ← `snapshot.stack`, `ctx.deps` ← `snapshot.deps`. **The snapshot's own `overrides` are authoritative** (re-applied to the restored state — `prepare_context`'s stamp ran against the soon-discarded empty state). This restore **precedes `before_node`** — seams never see the empty wire state. If `basestate[X]` is absent after a confirmed barrier (impossible by §4.1's basestate-first ordering, but the close reads it anyway) → **floor `calf.fanout.aborted` with `details.reason="basestate_missing"`** (⚠️ CORRECTION (PR-6 / 2026-06-17): the implemented vocabulary is the single `FaultTypes.FANOUT_ABORTED = "calf.fanout.aborted"` error_type discriminated by `details.reason ∈ {store_unavailable, basestate_missing, reentry_failed, dispatch_failed}` — **NOT** a distinct `calf.fanout.basestate_missing` error_type; see the fault-rail spec's §6.8 FANOUT_ABORTED CORRECTION), never `AttributeError`.
...
> **⚠️ CORRECTION (PR-6 / 2026-06-17):** the "exactly once" invariant holds **only under a writable store**. If the durable store is **unavailable** (a terminal reader death — `barrier()==False` with `status=='failed'`, the §5.1 abort trigger), the abort's tombstone write also fails, so each still-pending sibling that arrives during the outage independently fails-fast → aborts → escalates. With N pending siblings the caller can receive up to **N** `calf.fanout.aborted` faults for one batch. This is an **accepted operational residual** — a duplicate / over-delivery of the abort escalation (NOT a loss; v1 carries no exactly-once dedup state): the in-node design deliberately holds **zero in-process batch state**, so there is no cheap in-process dedup of the escalation. Recorded in fault-rail §13's residual-loss-windows enumeration.
...
> **⚠️ CORRECTION (PR-6 / 2026-06-17):** the "During a fold" and re-entry-publish-failure aborts above source the caller from the **INBOUND delivery's PRE-MUTATION stack** — the handler's `snapshot` (the marked fan-out frame on top carries the caller's `callback_topic`) — **NOT** from `basestate[X].snapshot.stack`. `basestate` is `None` precisely in the `basestate_missing` abort, so it must not be the caller source; the inbound stack is always available and reuses the `_BatchFaulted → _publish_fault(snapshot)` path (base.py carries a "no basestate read" comment at the abort). This is the same abort-source decision recorded in the fault-rail spec's §6.8 CORRECTION.
...
- **§6.8** — `_aggregate` is the durable in-node fold; closure is a self-published re-entry; `_BatchFaulted` *returned* → escalate, bypassing `on_node_error`. Flip the code-sketch comments ("the AGGREGATOR escalates … the node must not also fault directly"; "strays floor at the AGGREGATOR") **and** note `_publish_abort`'s *semantics* change (publish-a-record-the-aggregator-reads → in-node tombstone-both-tables-and-escalate, §4.4). **Closure recognition is implemented in a dedicated `_classify_fanout` method** (`in_reply_to == current_frame.frame_id` ⇒ closure re-entry → routed to the close path), **NOT as an arm on the sealed `_stray_check`** (which stays unchanged). *(⚠️ CORRECTION (PR-6 / 2026-06-17): this checklist item originally prescribed adding a closure-recognition arm to the sealed `_stray_check` — the implementation deliberately did NOT do that. The self-published closure re-entry is a shape-valid `return` + `ReturnMessage`, so it **passes** `_stray_check`'s kind↔slot-shape agreement test and is then classified as a re-entry by `_classify_fanout` — stray-exempt by **routing**, not by a new check on `_stray_check`.)*
