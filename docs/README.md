...
- **[How to handle errors and faults](docs/error-handling.md)** — recover from a failed tool with an agent's `on_tool_error` (or `surface_to_model()` to show the model every failure), from any callee with `on_callee_error`, from your own body with `on_node_error`, mint typed faults with `NodeFaultError`, and inspect an `ErrorReport`.
