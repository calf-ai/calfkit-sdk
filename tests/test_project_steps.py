...
from calfkit.models.step import AgentMessageEvent, ToolResultEvent
...
class TestStepPreamble:
    """The new AgentMessageEvent preamble extractor (spec §3.2): the FINAL ModelResponse's TextPart
    text only, excluding thinking/tool-call parts; empty ⇒ no AgentMessageEvent."""
...
class TestProjectStepsToolNode:
    def test_returncall_success_becomes_toolresult(self) -> None:
        ...
        assert len(events) == 1
        tr = events[0]
        assert isinstance(tr, ToolResultEvent)
...
class TestProjectStepsAgentNode:
    def test_non_returncall_output_returns_the_draft(self) -> None:
        agent = _agent_node()
        ctx = SessionRunContext(state=State(), deps={})
        ctx._step_draft = [AgentMessageEvent(parts=[TextPart(text="thinking")])]
    ...
    def test_inner_returncall_peer_consult_becomes_toolresult(self) -> None:
        # an agent answering as a consulted peer (inner-frame ReturnCall, depth>1) -> ToolResultEvent keyed by
        ...
        assert len(events) == 1 and isinstance(events[0], ToolResultEvent)
